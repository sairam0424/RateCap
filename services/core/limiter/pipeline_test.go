...
func TestPipeline_LaterTierSkipDoesNotClobberEarlierTierTier(t *testing.T) {
	tier1 := &fakeTier{decision: limiter.Decision{Action: limiter.ALLOW, Tier: "rate_limiter"}}
	tier2 := &fakeTier{decision: limiter.Decision{Action: limiter.ALLOW}}
	tier3 := &fakeTier{decision: limiter.Decision{Action: limiter.ALLOW}}

	p := limiter.NewPipeline(tier1, tier2, tier3)
	d, err := p.Check(context.Background(), limiter.Request{Key: "user-1", SkipReservations: true})

	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if d.Tier != "rate_limiter" {
		t.Errorf(`expected the skipped later tiers' empty Tier to not clobber tier1's "rate_limiter", got %q`, d.Tier)
	}
}
