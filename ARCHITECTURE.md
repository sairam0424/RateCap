...
## Helm chart distribution (v2.11.0)

`publish-release.yml` publishes the Helm chart as an OCI artifact — `helm package` + `helm push` to `oci://ghcr.io/sairam0424/charts/ratecap` — reusing the same GHCR registry and `GITHUB_TOKEN` credential the container images already publish through, rather than adding a second distribution mechanism. This replaces `helm/chart-releaser-action`, which required a `gh-pages` branch that has never existed on this repo (every release with chart changes since v2.4.0 silently failed at that step); OCI has been Helm's default, non-experimental chart-distribution model since v3.8.0, and `chart-releaser` itself still has no native OCI push support (upstream `helm/chart-releaser#622`, open/unmerged). Each pushed chart digest is signed with `cosign` (keyless) and carries a build-provenance attestation (`actions/attest-build-provenance`); `deploy/helm/ratecap/artifacthub-repo.yml` is pushed as a sibling `:artifacthub.io`-tagged OCI artifact via `oras`, the mechanism Artifact Hub documents for discovering and verifying OCI-based Helm repositories — distinct from the old `gh-pages`/`index.yaml` model, not a config tweak to it.
