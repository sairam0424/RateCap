...
- `Config.Validate()` (`services/core/config/config.go`) checks Tier 1's `default_rate`/`default_burst`, the concurrency-limiter/fleet-shedder fields, and queueing. The top-level `sync_rate` field (once vestigial — parsed but never consumed by any runtime code) was deleted entirely rather than wired up or validated; see `docs/superpowers/specs/2026-07-15-tier-4-worker-shedder-design.md` for why the core-to-sidecar config-sync mechanism it named was never built.
