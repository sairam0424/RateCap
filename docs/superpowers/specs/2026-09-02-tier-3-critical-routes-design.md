...
- **Correction, post-implementation**: this decision originally omitted a debounce window, reasoning it was purely an efficiency concern not worth the complexity for a low-frequency file. That was wrong — `services/core/config/watcher.go`'s `debounceWindow` (config/watcher.go:14) exists for a correctness reason, not an efficiency one: `os.WriteFile`'s truncate-then-write can be observed as two separate fsnotify events, the first against a transiently-empty file, which parses as valid-but-empty YAML (no error) — silently defeating the "keep last-known-good" guarantee before the real content ever arrives. This was caught by a genuine CI failure (not a test-design flaw) and fixed by adding the same 50ms `debounceWindow` pattern `config/watcher.go` already uses, plus a deterministic regression test (`TestWatch_SurvivesTruncateThenWriteRace`) that reproduces the exact race. `tlsconfig/reload.go` has the same gap (no debounce) and is unfixed — flagged as a follow-up, not touched here since it isn't currently exhibiting the bug.
