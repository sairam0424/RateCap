...
- The `deploy/sampleapp` demo's `/fleet-demo` endpoint exercises this exact header with no additional protection, but its blast radius is **larger** than `/slow-report`'s accepted demo risk profile, not equivalent to it: `ConcurrencyLimiter` (Tier 2, exercised by `/slow-report`) keys its capacity per request key, so an unauthenticated caller hammering `/slow-report` can only exhaust the concurrency budget tied to its own key. `FleetShedder` (Tier 3, exercised by `/fleet-demo`) keys its capacity globally — every request, regardless of caller or request key, is checked against the single shared `"fleet"` key. An unauthenticated caller hitting `/fleet-demo` with `x-ratecap-priority: critical` therefore consumes shared, fleet-wide reserved capacity, which can affect every other caller's requests across the whole demo stack, not just its own. Treat this as a global/shared-capacity risk, distinct from and larger than `/slow-report`'s per-key risk.
